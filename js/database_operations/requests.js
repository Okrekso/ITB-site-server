...
function createRequest(
  ...
  db.query(
    `INSERT INTO Requests(Name,Email,Pass,Stud_group)VALUES('${values.Name}','${
      values.Email
    }','${values.Pass}','${values.Stud_group}')`,
    (res, err) => {
      callback(err);
    }
  );
...
function declineUser(RequestID) {
  db.query(`SELECT Email FROM Requests WHERE ID=${RequestID}`, (res, err) => {
    if (res[0]!=undefined && res.length > 0)
      db.query(`DELETE FROM Requests WHERE ID=${RequestID}`, (result, err) => {
